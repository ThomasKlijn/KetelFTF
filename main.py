...
# Keywoordenlijst (voorbeeld, zou ook uit bestand kunnen komen)
keyword_dict = {
...
async def process_excel(file: UploadFile = File(...)):
    ...
    df_prod["combined_text"] = df_prod[tekstkolommen].apply(lambda r: " ".join([str(x) for x in r]), axis=1).str.lower()

    # Check keywoorden eerst (case insensitive)
    ...
    df_prod["Ketel gerelateerd keyword"] = df_prod["combined_text"].apply(check_keywords)

    # Vul ketel gerelateerd op basis keywords waar mogelijk, anders model
    mask_keyword = df_prod["Ketel gerelateerd keyword"].notnull()
    df_prod.loc[mask_keyword, "Ketel gerelateerd"] = df_prod.loc[mask_keyword, "Ketel gerelateerd keyword"]

    # Voor de overige rijen ketel voorspellen
    mask_model = ~mask_keyword
    ...
    df_prod.loc[mask_keyword, "Ketel zekerheid"] = 1.0  # Zekerheid 100% voor keywords

    # FTF voorspellen waar ketel 'j' is
    ...
    if not df_ftf.empty:
        ...
        df_ftf["FTF"] = np.where(y_proba_f.max(axis=1) > 0.6, y_pred_f.astype(str), "")
        ...
        df_prod["FTF"] = ""
        df_prod["FTF zekerheid"] = 0.0

    # Excel output met kleurcodering
        ...
        for col_idx, col_name in enumerate(df_prod.columns, start=1):
            ...
            if col_name == "Ketel gerelateerd":
                if row["Ketel zekerheid"] > 0.6:
                    ...
                    cell.fill = oranje
            if col_name == "FTF":
                if isinstance(row["FTF zekerheid"], float):
